...
public class SwingDisplay extends JPanel {
    ...
    public void refresh(String time, int totalArrivals) {
        this.time = time;
    ...
    // ============================================================
    //  TRACE DES LIAISONS (source unique : pathPoints)
    // ============================================================
    ...
    // Renvoie les points du trace entre deux gares :
    // 2 points (ligne droite) ou 3 points (coude en L).
    // Utilise pour le dessin ET pour le placement des vehicules.
    private Point[] pathPoints(Station from, Station to) {
        ...
        if (!down && !side) {
            return new Point[]{a, b};   // ligne droite
        }

        Point haut = (a.y <= b.y) ? a : b;
        Point bas  = (a.y <= b.y) ? b : a;
        Point corner = down ? new Point(haut.x, bas.y)   // descente puis horizontale
                : new Point(bas.x, haut.y);  // horizontale puis descente

        // on garde l'ordre from -> to pour interpoler dans le bon sens
    ...
    // position pixel d'un vehicule : interpolee LE LONG DU TRACE si en transit,
    // empilee pres de la gare si a quai
    ...
    // Position a la fraction "ratio" (0..1) le long d'une polyligne.
    private Point pointAlong(Point[] pts, double ratio) {
        double total = 0;
        for (int i = 0; i < pts.length - 1; i++) {
            total += segLength(pts[i], pts[i + 1]);
        }
    ...
    private void drawDashboard(Graphics2D g2) {
        ...
        y += 20; g2.drawString("  arrives (total) : " + totalArrivals, x, y);
