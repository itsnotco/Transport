...
public class SwingDisplay extends JPanel {
    ...
    private Point[] pathPoints(Station from, Station to) {
        ...
        Point corner;
        if (down) {
            corner = new Point(haut.x, bas.y);
        } else {
            corner = new Point(bas.x, haut.y);
        }
    ...
    private Point vehiclePosition(Vehicle v, Map<String, Integer> stacked) {
        ...
        if (travel <= 0) {
            ...
            ratio = (travel - v.getTimeUntilNextStation()) / travel;
        }
        if (ratio < 0) ratio = 0;
        if (ratio > 1) ratio = 1;
    ...
    private String trackedDetail(Passenger p) {
        ...
        if (p.getState() == PassengerState.WAITING) {
            ...
            String toName;
            if (leg != null) {
                vehName = leg.getVehicleId();
                toName = leg.getTo().getName();
            } else {
                vehName = "?";
                toName = "?";
            }
            pos = "dans " + vehName + " -> " + toName;
        } else {
